...
def prefs(cfg):
    """Notification settings. Defaults: due reminders the day before and the morning of, plus new-stuff and start-day notices."""
    when = cfg.get("notify_due_when")
    return {"due": cfg.get("notify_due", True), "new": cfg.get("notify_new", True), "start": cfg.get("notify_start", True),
            "when": [w for w in (when if isinstance(when, list) else ["day_before", "morning"]) if w in WHEN_CHOICES]}
...
def check(cfg=None, now=None, send=None):
    ...
    items = [i for i in data.get("items", []) if i.get("uid")]
    anns = data.get("announcements", [])
    ann_key = lambda a: a.get("url") or (a.get("course", "") + "|" + a.get("title", ""))
    now_seen = {"items": {i["uid"] for i in items}, "ann": {ann_key(a) for a in anns},
                "graded": {i["uid"] for i in items if i.get("score") is not None}}
    seen = st.get("seen")
    # New things. The first check only takes stock, so setting Semester up doesn't set off a flood.
    if seen is not None and p["new"]:
    ...
    if p["due"]:
        for i in items:
            ...
            if now >= due:
                continue
            morning = due.astimezone().replace(hour=MORNING_HOUR, minute=0, second=0, microsecond=0)
    ...
    if p["start"]:
        ...
        for i in items:
            if (i.get("start") and not i.get("submitted") and local.hour >= MORNING_HOUR
