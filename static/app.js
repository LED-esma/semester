  ...
  document.querySelectorAll('.nav').forEach(t => t.addEventListener('click', () => {
    ...
    document.dispatchEvent(new CustomEvent('tabchange', { detail: t.dataset.p }));
  }));
  ...
  const autoOn = $id('autoOn');
  autoOn.checked = (LS.getItem('semester.autoOn') || 'on') === 'on';
  autoOn.addEventListener('change', () => { LS.setItem('semester.autoOn', autoOn.checked ? 'on' : 'off');
    savePref({ autorefresh: autoOn.checked }); });
  ...
  bgNotify.addEventListener('change', async () => {
    ...
    try {
      const r = await fetch('/api/notify', { method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ enabled: bgNotify.checked, interval: 60 }) });
  ...
  // Open the tab they were on before a background refresh, otherwise their default tab
  const returnTab = sessionStorage.getItem('semester.returnTab'); sessionStorage.removeItem('semester.returnTab');
  const defTab = returnTab || LS.getItem('semester.defaultTab');
  ...
  // ---- Filters: one bar for the Week board, To-Do, and Discussions (class chips + type, due, status) ----
  const FILTER_KEY = 'semester.filters';
  const NO_FILTERS = { type: 'all', due: 'any', status: 'all' };
  let filters = (() => { try { return { ...NO_FILTERS, ...JSON.parse(LS.getItem(FILTER_KEY)) }; } catch (e) { return { ...NO_FILTERS }; } })();
  let searchQ = '';
  function passes(course, type, dueIso, done, title, useStatus) {
    if (loadHidden().has(course)) return false;
    if (filters.type !== 'all' && type !== filters.type) return false;
    if (useStatus && filters.status !== 'all' && (filters.status === 'done') !== !!done) return false;
    if (filters.due === 'nodate' && dueIso) return false;
    if (filters.due !== 'any' && filters.due !== 'nodate') {
      if (!dueIso) return false;
      const days = (new Date(dueIso).setHours(0, 0, 0, 0) - new Date().setHours(0, 0, 0, 0)) / 864e5;
      if (days < 0 || days > (filters.due === 'today' ? 0 : filters.due === 'week' ? 6 : 13)) return false;
    }
    return !searchQ || ((title || '') + ' ' + course).toLowerCase().includes(searchQ);
  }
  ...
  function renderChips() {
    ...
    DATA.forEach(d => { if (d.course && !(d.course in seen)) { seen[d.course] = d.color || '#999'; courses.push(d.course); } });  // every class with work
    ...
    courses.forEach(c => {
      ...
      chip.addEventListener('click', () => {
        ...
        saveHidden(h); renderChips(); refreshFilters();
      });
  ...
  function renderBoard() {
    ...
    const hidden = loadHidden();
    const visible = sortItems(KANBAN.filter(it => !it.done && passes(it.course, it.type, it.due, false, it.title)));
    const undated = [];
    visible.forEach(it => {
  ...
  function applyCardFilters() {
    ['todo', 'disc'].forEach(pid => {
      const panel = document.getElementById(pid); if (!panel) return;
      const cards = [...panel.querySelectorAll('.card[data-id]')];
      let shown = 0;
      cards.forEach(c => {
        const d = DATA[+c.dataset.id] || {};
        const ok = passes(d.course, d.type, d.dueIso, d.submitted, d.title, pid === 'disc');
        c.style.display = ok ? '' : 'none'; if (ok) shown++;
      });
      panel.querySelectorAll('section').forEach(sec => {
        const n = [...sec.querySelectorAll('.card[data-id]')].filter(c => c.style.display !== 'none').length;
        sec.style.display = n ? '' : 'none';
        const cnt = sec.querySelector('h2 .count'); if (cnt) cnt.textContent = n;
      });
      let none = panel.querySelector('.nomatch');
      if (!none && cards.length) {
        none = document.createElement('p'); none.className = 'empty nomatch';
        none.innerHTML = 'Nothing matches. <button class="linkish" type="button">Clear filters</button>';
        none.querySelector('button').onclick = clearFilters; panel.appendChild(none);
      }
      if (none) none.hidden = shown > 0;
    });
  }
  const fType = $id('fType'), fDue = $id('fDue'), fStatus = $id('fStatus'), fClear = $id('fClear');
  const isFiltered = () => filters.type !== 'all' || filters.due !== 'any' || filters.status !== 'all' || loadHidden().size > 0 || !!searchQ;
  function refreshFilters() {
    LS.setItem(FILTER_KEY, JSON.stringify(filters));
    fType.value = filters.type; fDue.value = filters.due; fStatus.value = filters.status;
    fClear.hidden = !isFiltered();
    renderBoard(); applyCardFilters();
  }
  function clearFilters() {
    filters = { ...NO_FILTERS }; saveHidden(new Set());
    const sb = $id('globalSearch'); if (sb && sb.value) { sb.value = ''; searchQ = ''; }
    renderChips(); refreshFilters();
  }
  [[fType, 'type'], [fDue, 'due'], [fStatus, 'status']].forEach(([el, k]) =>
    el.addEventListener('change', () => { filters[k] = el.value; refreshFilters(); }));
  fClear.addEventListener('click', clearFilters);
  // The bar only shows where it filters something; to do vs. done only matters on Discussions.
  const showBarFor = p => { $id('filterBar').hidden = !['week', 'todo', 'disc'].includes(p); fStatus.hidden = p !== 'disc'; };
  document.addEventListener('tabchange', e => showBarFor(e.detail));
  showBarFor((document.querySelector('.nav.active') || { dataset: { p: 'week' } }).dataset.p);
  refreshFilters();

  // Notifications: what to be told about, when, and from which classes. Saved to the config, so the
  // background job (when Semester is closed) uses exactly the same settings.
  const nDue = $id('nDue'), nStart = $id('nStart'), nQuiet = $id('nQuiet');
  const nNewA = $id('nNewA'), nNewN = $id('nNewN'), nNewG = $id('nNewG');
  const nMorning = $id('nMorning'), nQFrom = $id('nQFrom'), nQTo = $id('nQTo');
  const hourLabel = h => (h % 12 || 12) + (h < 12 ? ' AM' : ' PM');
  [nMorning, nQFrom, nQTo].forEach(sel => {
    for (let h = 0; h < 24; h++) { const o = document.createElement('option'); o.value = h; o.textContent = hourLabel(h); sel.appendChild(o); }
  });
  const lsOn = (k, dflt) => (LS.getItem(k) || (dflt ? 'on' : 'off')) === 'on';
  const lsNum = (k, dflt) => { const v = parseInt(LS.getItem(k), 10); return Number.isInteger(v) ? v : dflt; };
  const lsList = (k, dflt) => { try { const v = JSON.parse(LS.getItem(k)); return Array.isArray(v) ? v : dflt; } catch (e) { return dflt; } };
  let whenSel = lsList('semester.nWhen', ['day_before', 'morning']);
  let muted = lsList('semester.nMute', []);
  nDue.checked = lsOn('semester.nDue', true); nStart.checked = lsOn('semester.nStart', true);
  nQuiet.checked = lsOn('semester.nQuiet', false);
  nNewA.checked = lsOn('semester.nNewA', true); nNewN.checked = lsOn('semester.nNewN', true); nNewG.checked = lsOn('semester.nNewG', true);
  nMorning.value = lsNum('semester.nMorning', 8); nQFrom.value = lsNum('semester.nQFrom', 22); nQTo.value = lsNum('semester.nQTo', 8);
  document.querySelectorAll('[data-when]').forEach(cb => { cb.checked = whenSel.includes(cb.dataset.when); });

  const minutesLabel = m => (m % 1440 === 0 ? (m / 1440) + (m === 1440 ? ' day' : ' days') : (m / 60) + (m === 60 ? ' hour' : ' hours')) + ' before';
  function renderCustomTimes() {
    const box = $id('nCustom'); box.innerHTML = '';
    whenSel.filter(w => w.startsWith('minutes:')).forEach(w => {
      const chip = document.createElement('span'); chip.className = 'chip'; chip.title = 'Remove';
      chip.textContent = minutesLabel(+w.slice(8)) + ' ×';
      chip.addEventListener('click', () => { whenSel = whenSel.filter(x => x !== w); renderCustomTimes(); saveNotify(); });
      box.appendChild(chip);
    });
  }
  function renderMuteChips() {
    const box = $id('nMute'); box.innerHTML = '';
    const colors = {};
    DATA.forEach(d => { if (d.course && !(d.course in colors)) colors[d.course] = d.color || '#999'; });
    Object.keys(colors).forEach(c => {
      const chip = document.createElement('span'); chip.className = 'chip' + (muted.includes(c) ? ' off' : '');
      chip.innerHTML = '<span class="cdot" style="background:' + colors[c] + '"></span>' + esc(c);
      chip.addEventListener('click', () => {
        muted = muted.includes(c) ? muted.filter(x => x !== c) : muted.concat(c);
        renderMuteChips(); saveNotify();
      });
      box.appendChild(chip);
    });
  }
  const syncNotifyRows = () => {
    $id('nWhenRow').style.display = nDue.checked ? '' : 'none';
    $id('nQuietRow').style.display = nQuiet.checked ? '' : 'none';
  };
  function saveNotify() {
    whenSel = [...document.querySelectorAll('[data-when]')].filter(cb => cb.checked).map(cb => cb.dataset.when)
      .concat(whenSel.filter(w => w.startsWith('minutes:')));
    LS.setItem('semester.nWhen', JSON.stringify(whenSel));
    LS.setItem('semester.nMute', JSON.stringify(muted));
    LS.setItem('semester.nMorning', nMorning.value); LS.setItem('semester.nQFrom', nQFrom.value); LS.setItem('semester.nQTo', nQTo.value);
    [['semester.nDue', nDue], ['semester.nStart', nStart], ['semester.nQuiet', nQuiet],
     ['semester.nNewA', nNewA], ['semester.nNewN', nNewN], ['semester.nNewG', nNewG]]
      .forEach(([k, el]) => LS.setItem(k, el.checked ? 'on' : 'off'));
    savePref({ notify_due: nDue.checked, notify_due_when: whenSel, notify_morning_hour: +nMorning.value,
      notify_new_assignments: nNewA.checked, notify_new_announcements: nNewN.checked, notify_new_grades: nNewG.checked,
      notify_start: nStart.checked, notify_quiet: nQuiet.checked, notify_quiet_from: +nQFrom.value,
      notify_quiet_to: +nQTo.value, notify_mute_courses: muted });
    syncNotifyRows();
  }
  [nDue, nStart, nQuiet, nNewA, nNewN, nNewG, nMorning, nQFrom, nQTo, ...document.querySelectorAll('[data-when]')]
    .forEach(el => el.addEventListener('change', saveNotify));
  $id('nAdd').addEventListener('click', () => {
    const m = Math.round((parseInt($id('nAmount').value, 10) || 0) * +$id('nUnit').value);
    if (!m || m > 20160) return;  // up to two weeks ahead
    if (!whenSel.includes('minutes:' + m)) whenSel.push('minutes:' + m);
    renderCustomTimes(); saveNotify();
  });
  renderCustomTimes(); renderMuteChips(); syncNotifyRows();
  $id('nTest').addEventListener('click', async () => {
    try {
      const d = await (await fetch('/api/notify/test', { method: 'POST' })).json();
      showToast(d.ok ? 'Sent. If nothing appeared, allow notifications for Semester in your system settings.' : 'Notifications need the installed app.');
    } catch (e) { showToast('Notifications need the installed app.'); }
  });
  ...
  if (location.protocol.indexOf('http') === 0 && $id('icsFeedRow')) {
    ...
    $id('icsCopy').addEventListener('click', async () => {
      try { await navigator.clipboard.writeText(feedUrl); showToast('Copied. Apple Calendar: File → New Calendar Subscription. Google Calendar: Other calendars → From URL.'); }
      catch (e) { prompt('Copy this URL:', feedUrl); }
    });
  }

  // ---- One-time tips: explain a feature the first time it's in view, then get out of the way ----
  const TIPS = [
    { id: 'plan', tab: 'week',  // points at the always-visible button, and only while cards wait in the tray
      anchor: () => document.querySelector('#weektray .kcard') && document.getElementById('autoBtn'),
      text: 'Drag a card onto a day to plan when you’ll work on it. Plan my week can do it for you.' },
    { id: 'start', tab: 'todo', anchor: () => document.querySelector('#todo .card .start'),
      text: 'Green start dates are suggested days to begin, so work doesn’t pile up.' },
    { id: 'whatif', tab: 'grades', anchor: () => [...document.querySelectorAll('#gradesBox input:not([data-target])')].find(i => !i.value),
      text: 'Type a score into any ungraded row to see what-if grades. Change the target to see what you need.' },
  ];
  const tipsOn = () => (LS.getItem('semester.tips') || 'on') === 'on';
  // A tip retires for good once dismissed or once its feature is used; closing it any other way rests it for 30 days.
  const SNOOZE_MS = 30 * 864e5;
  const tipState = () => { try { const v = JSON.parse(LS.getItem('semester.tipsSeen'));
    return Array.isArray(v) ? Object.fromEntries(v.map(id => [id, 0])) : (v || {}); } catch (e) { return {}; } };
  const tipResting = id => { const v = tipState()[id]; return v === 0 || v > Date.now(); };
  function retireTip(id, snooze) {
    const st = tipState(); if (st[id] === 0) return;
    st[id] = snooze ? Date.now() + SNOOZE_MS : 0; LS.setItem('semester.tipsSeen', JSON.stringify(st));
  }
  let tipEl = null, tipAnchor = null;
  function placeTip() {
    if (!tipEl) return;
    const r = tipAnchor.getBoundingClientRect(), w = tipEl.offsetWidth, h = tipEl.offsetHeight;
    tipEl.style.visibility = (r.bottom < 0 || r.top > innerHeight || !tipAnchor.offsetParent) ? 'hidden' : '';
    const left = Math.max(12, Math.min(r.left, innerWidth - w - 12));
    const below = r.bottom + h + 16 < innerHeight;
    tipEl.style.left = left + 'px';
    tipEl.style.top = (below ? r.bottom + 10 : r.top - h - 10) + 'px';
    tipEl.classList.toggle('above', !below);
    tipEl.style.setProperty('--tip-x', Math.max(12, Math.min(w - 22, r.left + Math.min(r.width, 40) / 2 - left)) + 'px');
  }
  function hideTip(how) {  // 'done' retires it, 'snooze' rests it, nothing just hides it
    if (!tipEl) return;
    if (how) retireTip(tipEl.dataset.id, how === 'snooze');
    tipEl.remove(); tipEl = tipAnchor = null;
  }
  const tipUsed = id => { retireTip(id, false); if (tipEl && tipEl.dataset.id === id) hideTip(); };
  function showTipFor(tab) {
    hideTip('snooze');
    if (!tipsOn() || modal.classList.contains('open') || sessionStorage.getItem('semester.tipShown')) return;  // one tip per launch
    const tip = TIPS.find(t => t.tab === tab && !tipResting(t.id));
    const a = tip && tip.anchor();
    if (!a || !a.offsetParent) return;  // the feature isn't on screen right now
    tipEl = document.createElement('div'); tipEl.className = 'tip'; tipEl.dataset.id = tip.id; tipEl.setAttribute('role', 'status');
    tipEl.innerHTML = '<div></div><div class="tip-row"><button type="button">Got it</button></div>';
    tipEl.firstChild.textContent = tip.text;
    tipEl.querySelector('button').onclick = () => hideTip('done');
    tipAnchor = a; document.body.appendChild(tipEl); placeTip();
    sessionStorage.setItem('semester.tipShown', '1');
  }
  addEventListener('scroll', placeTip, true); addEventListener('resize', placeTip);
  addEventListener('pagehide', () => hideTip('snooze'));
  // Already using the feature? Then the tip has nothing to teach.
  document.addEventListener('drop', e => { if (e.target.closest && e.target.closest('#weekboard, #weektray')) tipUsed('plan'); }, true);
  $id('autoBtn').addEventListener('click', () => tipUsed('plan'));
  document.addEventListener('input', e => { if (e.target.closest && e.target.closest('#gradesBox')) tipUsed('whatif'); }, true);
  document.addEventListener('keydown', e => { if (e.key === 'Escape' && tipEl) hideTip('done'); });
  document.addEventListener('tabchange', e => { hideTip('snooze'); setTimeout(() => showTipFor(e.detail), 450); });
  setTimeout(() => { const n = document.querySelector('.nav.active'); showTipFor(n ? n.dataset.p : 'week'); }, 1200);
  const tipsToggle = $id('showTips');
  if (tipsToggle) {
    tipsToggle.checked = tipsOn();
    tipsToggle.addEventListener('change', () => {
      LS.setItem('semester.tips', tipsToggle.checked ? 'on' : 'off');
      if (tipsToggle.checked) { LS.removeItem('semester.tipsSeen'); sessionStorage.removeItem('semester.tipShown');
        showToast('Tips will show again as you use Semester.'); }
      else hideTip();
    });
  }

  // ---- Account: who's connected, and log out (a few seconds to undo instead of an "are you sure?") ----
  fetch('/api/account', { cache: 'no-store' }).then(r => r.json()).then(a => {
    if (!a.ok) return;
    $id('accountGroup').style.display = '';
    $id('acctName').textContent = a.name || 'Canvas account';
    $id('acctSchool').textContent = 'Connected to ' + a.school;
  }).catch(() => {});
  $id('logoutBtn').addEventListener('click', () => {
    const btn = $id('logoutBtn'); btn.disabled = true;
    const t = setTimeout(async () => {
      try { await fetch('/api/logout', { method: 'POST' }); LS.setItem('semester.bgNotify', 'off'); location.href = '/setup'; }
      catch (e) { btn.disabled = false; showToast('Couldn’t log out. Try again.'); }
    }, 5000);
    showToast('Logging out. Click to undo.', () => { clearTimeout(t); btn.disabled = false;
      setTimeout(() => showToast('Still logged in.'), 50); });
  });
  ...
  const reloadHere = () => { const n = document.querySelector('.nav.active');
    if (n) sessionStorage.setItem('semester.returnTab', n.dataset.p); location.reload(); };
  async function checkUpdates() {
    if ((LS.getItem('semester.autoOn') || 'on') !== 'on') return;
    try {
      const s = await (await fetch('/api/status', { cache: 'no-store' })).json();
      if (!(s.built && s.built > PAGE_BUILT)) return;
      const busy = modal.classList.contains('open') || /INPUT|TEXTAREA|SELECT/.test(document.activeElement.tagName)
        || document.querySelector('.focusbar.show');
      if (!busy && Date.now() - lastInput > 20000) return reloadHere();
      if (!freshShown) { freshShown = true; showToast('Fresh data from Canvas — click to refresh.', reloadHere); }
    } catch (e) {}
  }
  setInterval(checkUpdates, 60 * 1000);  // a local call; Canvas is only asked by the server's change check
  // Back after a couple of minutes away: ask for a (nearly free) change check right away.
  let awaySince = 0;
  const away = () => { if (!awaySince) awaySince = Date.now(); };
  const back = () => {
    if (awaySince && Date.now() - awaySince > 2 * 60 * 1000)
      fetch('/api/check', { method: 'POST' }).then(() => setTimeout(checkUpdates, 6000)).catch(() => {});
    awaySince = 0;
  };
  addEventListener('blur', away); addEventListener('focus', back);
...
if (searchBox) {
  // Typing narrows what's on screen (alongside the filters) and suggests places to jump to.
  searchBox.addEventListener('input', () => {
    const q = searchBox.value.trim().toLowerCase();
    searchQ = q; refreshFilters();
    document.querySelectorAll('.ann, .course-card, .modrow, .classcard').forEach(el => {
      el.style.display = (!q || el.textContent.toLowerCase().includes(q)) ? '' : 'none';
    });
    document.querySelectorAll('section').forEach(sec => {
      const anns = sec.querySelectorAll('.ann'); if (!anns.length) return;
      sec.style.display = (!q || [...anns].some(el => el.style.display !== 'none')) ? '' : 'none';
    });
    renderSuggest(q);
  });
  ...
  let sugs = [], sugPick = -1;
  const goTab = p => { const n = document.querySelector('.nav[data-p="' + p + '"]'); if (n) n.click(); };
  function buildSuggestions(q) {
    if (q.length < 2) return [];
    const out = [], starts = s => (s || '').toLowerCase().startsWith(q), has = s => (s || '').toLowerCase().includes(q);
    Object.entries(TITLES).forEach(([p, name]) => {
      const n = document.querySelector('.nav[data-p="' + p + '"]');
      if (has(name) && n && n.style.display !== 'none') out.push({ rank: starts(name) ? 0 : 1, label: name, meta: 'Section', go: () => goTab(p) });
    });
    CLASSES.forEach((c, i) => { if (has(c.name)) out.push({ rank: starts(c.name) ? 1 : 2, label: c.name, meta: 'Class',
      go: () => { goTab('classes'); clsTab = 'stream'; renderClassDetail(i); } }); });
    const seenUid = new Set();  // an item is listed once per screen it appears on; suggest it only once
    DATA.forEach((d, i) => {
      if (!(has(d.title) || has(d.course)) || (d.uid && seenUid.has(d.uid))) return;
      seenUid.add(d.uid);
      out.push({ rank: (starts(d.title) ? 2 : 3) + (d.submitted ? 2 : 0), label: d.title,
                 meta: d.course + (d.due ? ' · due ' + d.due : '') + (d.submitted ? ' · done' : ''), go: () => openModal(i) });
    });
    document.querySelectorAll('#ann .ann').forEach(el => {
      const t = (el.querySelector('.title') || el).textContent.trim();
      if (has(t)) out.push({ rank: 4, label: t, meta: 'Announcement', go: () => {
        goTab('ann'); el.scrollIntoView({ block: 'center' }); el.classList.add('flash'); setTimeout(() => el.classList.remove('flash'), 1600); } });
    });
  ...
  function drawSuggest() {
    sugBox.innerHTML = '';
    sugs.forEach((s, k) => {
      const li = document.createElement('li'); li.setAttribute('role', 'option'); li.setAttribute('aria-selected', k === sugPick);
      li.innerHTML = '<span></span><span class="d"></span>'; li.firstChild.textContent = s.label; li.lastChild.textContent = s.meta;
      li.addEventListener('mousedown', e => { e.preventDefault(); pickSuggestion(k); });
      sugBox.appendChild(li);
    });
    sugBox.hidden = !sugs.length; searchBox.setAttribute('aria-expanded', !!sugs.length);
  }
  function renderSuggest(q) { sugs = buildSuggestions(q); sugPick = sugs.length ? 0 : -1; drawSuggest(); }
  function pickSuggestion(k) {
    const s = sugs[k]; if (!s) return;
    searchBox.value = ''; searchBox.dispatchEvent(new Event('input')); searchBox.blur();  // clear the narrowing, then go
    s.go();
  }
  searchBox.addEventListener('keydown', e => {
    if (sugBox.hidden) return;
    if (e.key === 'ArrowDown' || e.key === 'ArrowUp') {
      e.preventDefault(); sugPick = (sugPick + (e.key === 'ArrowDown' ? 1 : sugs.length - 1)) % sugs.length; drawSuggest();
    } else if (e.key === 'Enter') { e.preventDefault(); pickSuggestion(Math.max(sugPick, 0)); }
