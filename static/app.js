  ...
  // Notifications: what to be told about, and when. The app and the background job read these from the config.
  const nDue = $id('nDue'), nNew = $id('nNew'), nStart = $id('nStart'), nWhen = [...document.querySelectorAll('[data-when]')];
  const lsOn = (k, dflt) => (LS.getItem(k) || (dflt ? 'on' : 'off')) === 'on';
  let whenSel; try { whenSel = JSON.parse(LS.getItem('semester.nWhen')) || ['day_before', 'morning']; } catch (e) { whenSel = ['day_before', 'morning']; }
  nDue.checked = lsOn('semester.nDue', true); nNew.checked = lsOn('semester.nNew', true); nStart.checked = lsOn('semester.nStart', true);
  nWhen.forEach(cb => { cb.checked = whenSel.includes(cb.dataset.when); });
  const syncWhenRow = () => { $id('nWhenRow').style.display = nDue.checked ? '' : 'none'; };
  function saveNotify() {
    whenSel = nWhen.filter(cb => cb.checked).map(cb => cb.dataset.when);
    LS.setItem('semester.nDue', nDue.checked ? 'on' : 'off'); LS.setItem('semester.nNew', nNew.checked ? 'on' : 'off');
    LS.setItem('semester.nStart', nStart.checked ? 'on' : 'off'); LS.setItem('semester.nWhen', JSON.stringify(whenSel));
    savePref({ notify_due: nDue.checked, notify_due_when: whenSel, notify_new: nNew.checked, notify_start: nStart.checked });
    syncWhenRow();
  }
  [nDue, nNew, nStart, ...nWhen].forEach(el => el.addEventListener('change', saveNotify));
  syncWhenRow();
