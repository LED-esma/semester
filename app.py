...
def self_update():
    ...
    if sys.platform == "darwin":
        url, app_path = _asset_url(d, "Semester-macOS.zip"), _app_bundle_path()
        ...
        if not os.path.isdir(newapp): return "error"
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .reauth { display:none; text-align:left; background:#fff4e5; color:#7a4a00;
    border:1px solid #f0c98a; border-radius:10px; padding:11px 14px; font-size:14px;
    line-height:1.5; margin:0 auto 16px; max-width:430px; }
  .reauth.on { display:block; }
  .reauth-status { min-height:22px; font-size:14px; margin:-8px 0 12px; }
</style></head>
<body>
<div class="wiz">
  ...
  <section class="screen" data-i="1">
    ...
    <input id="url" type="text" placeholder="myschool.instructure.com" autocomplete="off" value="__URL__">
    <br><button class="cont" data-next>Continue</button>
  </section>

  <section class="screen" data-i="2">
    <div class="glyph">&#128273;</div>
    <h1 id="tokenHead">Get your access token</h1>
    <div class="reauth" id="reauthNote">Your Canvas token expired or was revoked, so Semester can&rsquo;t reach your courses. Generate a new one and paste it below &mdash; your settings, week plan, and notes all stay put.</div>
...
<script>
  ...
  if (window.__REAUTH__) {
    document.getElementById('reauthNote').classList.add('on');
    document.getElementById('tokenHead').textContent = 'Reconnect to Canvas';
    show(2);
    const scr = screens[2], tok = $('token');
    $('back').style.display = 'none'; dotsBox.style.display = 'none';
    scr.querySelector('p').style.display = 'none';
    const cont = scr.querySelector('[data-next]'); if (cont) cont.style.display = 'none';
    const getTok = $('openCanvas'); getTok.className = 'cont'; getTok.innerHTML = 'Get a new token &#8599;';
    tok.placeholder = 'Paste it here \u2014 it connects automatically';
    const status = document.createElement('div'); status.className = 'reauth-status'; tok.after(status);
    let timer = null, last = '';
    const tryToken = async () => {
      const t = tok.value.trim();
      if (t.length < 20 || t === last) return;
      last = t; status.style.color = '#6e6e73'; status.textContent = 'Checking\u2026';
      try {
        const r = await fetch('/api/reauth', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: t }) });
        const d = await r.json();
        if (d.ok) {
          status.style.color = '#1d8a3a';
          status.textContent = '\u2713 Reconnected' + (d.name ? ', ' + d.name.split(' ')[0] : '') + ' \u2014 opening your dashboard\u2026';
          setTimeout(() => { location.href = '/'; }, 700);
        } else { status.style.color = '#c0392b'; status.textContent = d.error || 'That token didn\u2019t work \u2014 copy it again.'; }
      } catch (e) { status.style.color = '#c0392b'; status.textContent = 'Couldn\u2019t reach Semester \u2014 try again.'; last = ''; }
    };
    tok.addEventListener('input', () => { clearTimeout(timer); timer = setTimeout(tryToken, 350); });
    tok.addEventListener('keydown', e => { if (e.key === 'Enter') { e.stopPropagation(); tryToken(); } });
  }
</script>
</body></html>"""


def with_reconnect_banner(html):
    """Cached dashboard + a slim reconnect bar, so their data stays usable while offline."""
    try:
    ...
    bar = ('<style>body{display:flex;flex-direction:column;height:100vh}'
           ...
           f'<span>Canvas disconnected &mdash; your access token expired. Showing data from {ts}.</span>'
           '<a href="/setup?reauth=1" style="background:#7a4a00;color:#fff;text-decoration:none;'
           'padding:6px 14px;border-radius:8px;font-weight:600">Reconnect</a></div>')
    ...
    def do_GET(self):
        ...
        if self.path == "/" or self.path.startswith("/dashboard"):
            ...
            if BUILD["status"] == "expired":
                if os.path.exists(cp.HTML_PATH):   # result first: keep showing their data
                    with open(cp.HTML_PATH, encoding="utf-8") as f:
                        return self._send(200, with_reconnect_banner(f.read()))
