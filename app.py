...
def mac_self_update():
    """Download the latest notarized macOS build and swap it in, then relaunch.
    User data lives in ~/.semester + the browser, so nothing is lost in the swap."""
    d = _release_json()
    url = next((a.get("browser_download_url") for a in d.get("assets", [])
                if a.get("name") == "Semester-macOS.zip"), None)
    app_path = _app_bundle_path()
    if not url or not app_path:
        webbrowser.open(d.get("html_url") or f"https://github.com/{GH_REPO}/releases")
        return "opened"
    ...
    if not os.path.isdir(newapp):
        return "error"
    helper = os.path.join(tmp, "swap.sh")
