...
VERSION = "1.3"  # keep in sync with the latest GitHub release tag
...
class Canvas:
    ...
    def files(self, course_id):
        return self.get(f"/courses/{course_id}/files", {"per_page": 100, "sort": "updated_at", "order": "desc"})

    def pages(self, course_id):
        return self.get(f"/courses/{course_id}/pages", {"per_page": 100, "sort": "updated_at", "order": "desc"})
...
def render_html(items, warnings, courses_info, announcements, colors, now, accent="#6366f1", grades=None, classes=None):
    ...
    # ---- AI export: per-section data (user picks which sections to download) ----
    def _exp_item(it):
        return {
            "title": it["title"], "course": clean_course_name(it["course"]),
            "due": it["due"], "due_local": fmt_due(it["due"]) if it["due"] else None,
            "suggested_start": it["start"], "points": it.get("points"),
            "status": "overdue" if it["bucket"] == "overdue" else ("no_due_date" if not it["due"] else "upcoming"),
            "due_from_title": bool(it.get("due_approx")), "url": it.get("url"),
        }
    _by_due = sorted(items, key=lambda x: (x["due"] is None, x["due"] or "~"))
    export_obj = {
        "instructions": ("This is my Canvas coursework, exported from my Semester app. The keys below are the "
                         "sections I chose to include (assignments, discussions, announcements, grades, courses). "
                         "Help me plan: prioritize by due date and workload, suggest what to start when, and flag "
                         "anything overdue or at risk. Dates are ISO 8601. 'due_from_title' = true means the "
                         "deadline was parsed from the assignment's title, so confirm it with me."),
        "generated": now.isoformat(),
        "today": now.astimezone().date().isoformat(),
        "assignments": [_exp_item(it) for it in _by_due if it["type"] == "assignment" and not it["submitted"]],
        "discussions": [_exp_item(it) for it in _by_due
                        if it["type"] == "discussion" and not it["submitted"] and it.get("points") is not None],
        "announcements": [{"course": clean_course_name(a["course"]), "title": a["title"],
                           "posted": a.get("posted"), "preview": a.get("preview")} for a in announcements],
        "grades": [{"course": clean_course_name(g["name"]), "score": g.get("score"), "grade": g.get("grade"),
                    "graded_items": [{"title": i["title"], "score": i["score"], "points": i["points"]}
                                     for i in g["items"] if i.get("graded")]}
                   for g in (grades or [])],
        "courses": [{"course": clean_course_name(c["name"]), "url": c.get("url"),
                     "pending": c.get("pending"), "syllabus": c.get("syllabus")} for c in courses_info],
    }
    export_json = json.dumps(export_obj, ensure_ascii=False, indent=2).replace("</", "<\\/")
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  /* Classes (Google Classroom-style) */
  .classgrid {{ display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 14px; margin-top: 8px; }}
  .classcard {{ background: var(--surface); border: 1px solid var(--border); border-radius: 12px;
          overflow: hidden; cursor: pointer; box-shadow: var(--shadow); transition: transform .08s ease; }}
  .classcard:hover {{ transform: translateY(-2px); }}
  .classcard .top {{ height: 70px; padding: 12px 16px; color: #fff; display: flex; flex-direction: column; justify-content: space-between; }}
  .classcard .top h3 {{ margin: 0; font-size: 16px; font-weight: 500; color: #fff; }}
  .classcard .bot {{ padding: 10px 16px; display: flex; justify-content: space-between; font-size: 13px; color: var(--dim); }}
  .classcard .grade {{ font-weight: 600; color: var(--text); }}
  .clstitle {{ display: flex; align-items: center; gap: 12px; margin: 4px 0 14px; }}
  .clsback {{ background: var(--surface); border: 1px solid var(--border); border-radius: 8px;
          color: var(--text); cursor: pointer; font: inherit; font-size: 13px; padding: 6px 12px; }}
  .clsback:hover {{ background: var(--hover); }}
  .clstabs {{ display: flex; gap: 4px; border-bottom: 1px solid var(--border); margin-bottom: 16px; flex-wrap: wrap; }}
  .clstab {{ background: none; border: none; border-bottom: 2px solid transparent; color: var(--dim);
          font: inherit; font-size: 14px; font-weight: 500; padding: 9px 14px; cursor: pointer; }}
  .clstab:hover {{ color: var(--text); }}
  .clstab.on {{ color: var(--text); border-bottom-color: var(--accent); }}
  .mod {{ background: var(--surface); border: 1px solid var(--border); border-radius: 10px; margin-bottom: 12px; overflow: hidden; }}
  .mod h4 {{ margin: 0; padding: 11px 15px; font-size: 14px; background: var(--surface-2); border-bottom: 1px solid var(--border); }}
  .modrow {{ display: flex; align-items: center; gap: 10px; padding: 9px 15px; font-size: 13px;
  ...
  .modrow .ic {{ width: 18px; color: var(--dim); flex: none; text-align: center; }}
  .modrow .mt {{ flex: 1; }}
  .modrow .md {{ color: var(--dim); font-size: 12px; white-space: nowrap; }}
  .matlist a {{ display: flex; justify-content: space-between; gap: 10px; padding: 9px 14px; font-size: 13px;
          text-decoration: none; color: inherit; border: 1px solid var(--border); border-radius: 8px; margin-bottom: 6px; background: var(--surface); }}
  .matlist a:hover {{ background: var(--hover); }}
  .matlist .sz {{ color: var(--dim); font-size: 12px; }}
  /* Settings */
...
<div class="app">
  ...
  <div class="panel" id="todo">{warn_html}{todo}</div>
    ...
    <div class="panel" id="settings">
      ...
      <div class="setgroup">
        <h2>Export for AI</h2>
        <p class="sd" style="margin:0 0 10px">Pick which sections to include, then download a JSON (with a ready-to-paste prompt) for any AI.</p>
        <div class="setrow"><div class="st">Assignments</div><label class="switch"><input type="checkbox" id="exp_assignments"><span class="track"></span></label></div>
        <div class="setrow"><div class="st">Discussions</div><label class="switch"><input type="checkbox" id="exp_discussions"><span class="track"></span></label></div>
        <div class="setrow"><div class="st">Announcements</div><label class="switch"><input type="checkbox" id="exp_announcements"><span class="track"></span></label></div>
        <div class="setrow"><div class="st">Grades</div><label class="switch"><input type="checkbox" id="exp_grades"><span class="track"></span></label></div>
        <div class="setrow"><div class="st">Courses &amp; syllabus</div><label class="switch"><input type="checkbox" id="exp_courses"><span class="track"></span></label></div>
        <div class="setrow"><div><div class="st">Download</div><div class="sd">Saves the selected sections as JSON.</div></div>
          <button class="dlbtn" id="aiExport">Download .json</button></div>
      </div>
      <div class="setgroup">
        <h2>About</h2>
...
<script type="application/json" id="exportdata">{export_json}</script>
<script>
  ...
  const ICON = {{ Assignment: '📝', Quiz: '❓', Discussion: '💬', Page: '📄', File: '📎', ExternalUrl: '🔗', ExternalTool: '🧩' }};
  let clsTab = 'stream';
  ...
  function renderClassDetail(i) {{
    ...
    if (clsTab === 'stream') {{
      ...
      if (c.quizzes.length) h += '<div class="mod"><h4>Quizzes</h4>' + c.quizzes.map(q => '<a class="modrow" href="' + (q.url || '#') + '" target="_blank"><span class="ic">❓</span><span class="mt">' + esc(q.title || '') + '</span>' + (q.due_local ? '<span class="md">due ' + esc(q.due_local) + '</span>' : '') + '</a>').join('') + '</div>';
