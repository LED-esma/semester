...
pyinstaller --windowed --onedir --name "Semester" --icon icon.icns --add-data "static:static" \
...
ditto -c -k --keepParent "$APP" "Semester-mac.zip"
xcrun notarytool submit "Semester-mac.zip" --keychain-profile "$PROFILE" --wait
echo "▸ Stapling ticket…"
xcrun stapler staple "$APP"
xcrun stapler validate "$APP"
ditto -c -k --keepParent "$APP" "Semester-mac.zip"
echo "✓ Done — Semester-mac.zip is signed + notarized and runs with no Gatekeeper warning."
