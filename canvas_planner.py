...
VERSION = "1.1"  # keep in sync with the latest GitHub release tag
...
<div class="app">
  ...
  <main class="content">
    ...
    <div class="panel" id="settings">
      ...
      <div class="setgroup">
        <h2>About</h2>
        <div class="setrow"><div><div class="st">Semester {VERSION}</div><div class="sd">Check GitHub for a newer version.</div></div>
          <button class="ghostbtn" id="checkUpdate">Check for updates</button></div>
      </div>
    </div>
...
<script>
  ...
  function autoSchedule(algo) {{
    ...
    const tray = KANBAN.filter(it => !hidden.has(it.course) && !(plan[it.uid] && dayKeys.includes(plan[it.uid])));
  ...
  async function checkUpdate(manual) {{
    try {{
      ...
      if (j.newer) {{
        ...
        bar.innerHTML = '🔔 Semester v' + j.latest + ' is available. <button id="updGo">' + (j.mode === 'git' ? 'Update now' : 'Download') + '</button><button class="x" id="updX">✕</button>';
        bar.classList.add('show');
        document.getElementById('updGo').onclick = applyUpdate;
