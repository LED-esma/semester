...
case "$(uname -s)" in
  Darwin)
    pyinstaller --windowed --name "Semester" --icon icon.icns --add-data "static:static" $COMMON app.py
    echo "✓ Built: dist/Semester.app" ;;
  Linux)
    pyinstaller --name semester --add-data "static:static" $COMMON app.py
    echo "✓ Built: dist/semester" ;;
  *)
    pyinstaller --windowed --name Semester --icon icon.ico --add-data "static;static" $COMMON app.py
