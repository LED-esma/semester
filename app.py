...
NOTIFY_KEYS = ("notify_due", "notify_due_when", "notify_new", "notify_start")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/reauth":
            return self._reauth()
        if self.path not in ("/api/save", "/api/logout", "/api/signin/start", "/api/notify/test", "/api/check", "/api/check_url", "/api/prefs", "/api/notify", "/api/canvas/done",
        ...
        if self.path == "/api/prefs":
            ...
            if not re.fullmatch(r"#[0-9a-fA-F]{6}", str(data.get("accent", "#000000"))):
                data.pop("accent")
            for k in ("notify_due", "notify_new", "notify_start"):
                if k in data:
                    data[k] = bool(data[k])
            if "notify_due_when" in data:
                w = data["notify_due_when"]
                data["notify_due_when"] = [x for x in w if x in ("day_before", "morning", "hours3")] if isinstance(w, list) else []
