...
def _notify(title, body):
    ...
    try:
        if system == "Darwin":
            ...
            # Windows' own toast API. The usual PowerShell module for this (BurntToast) isn't
            # installed on any normal machine, so reminders were simply silent there.
            ps = ('[Windows.UI.Notifications.ToastNotificationManager, Windows.UI.Notifications,'
                  ' ContentType=WindowsRuntime] > $null;'
                  '$x = [Windows.UI.Notifications.ToastNotificationManager]::GetTemplateContent('
                  '[Windows.UI.Notifications.ToastTemplateType]::ToastText02);'
                  '$t = $x.GetElementsByTagName("text");'
                  f'$t.Item(0).AppendChild($x.CreateTextNode({q(title)})) > $null;'
                  f'$t.Item(1).AppendChild($x.CreateTextNode({q(body)})) > $null;'
                  '[Windows.UI.Notifications.ToastNotificationManager]::CreateToastNotifier('
                  '"Semester").Show([Windows.UI.Notifications.ToastNotification]::new($x))')
...
def _config():
    try:
        cfg = json.load(open(cp.CONFIG_PATH, encoding="utf-8"))
...
def _due_phrase(due, now):
    d = due.astimezone()
    days, t = (d.date() - now.astimezone().date()).days, cp.strf(d, "%-I:%M %p")
...
def install(interval_min=60):
    if platform.system() == "Windows":
        ...
        r = subprocess.run(["schtasks", "/Create", "/F", "/TN", TASK, "/SC", "MINUTE",
                            "/MO", str(max(15, interval_min)), "/TR", run],
                           capture_output=True, text=True)
        if r.returncode != 0:
            return False, "Windows wouldn't schedule the reminder check. Semester still reminds you while it's open."
        return True, "Background reminders enabled."
    if platform.system() != "Darwin":
        return False, "Auto-scheduling is Linux-only by hand for now; see README."
...
def uninstall():
    if platform.system() == "Windows":
        subprocess.run(["schtasks", "/Delete", "/F", "/TN", TASK], check=False, capture_output=True)
