...
- **Filters and search.** Filter by class, type, and due date. Search suggests assignments, classes, and announcements as you type.
...
- **Notifications.** Reminders before things are due, at times you choose: the day before, the morning it's due
  (your hour), 3 hours before, or any time you add. Plus a heads-up for new assignments, announcements, and
  grades, each with its own switch. Set quiet hours, or mute a class, in Settings. Reminders keep
  working with the app closed on Mac and Windows.
...
- **Mac:** `Semester-mac.dmg`. Open it and drag Semester into Applications. Runs on Intel and Apple Silicon, macOS 11 or later.
...
- If your school signs in through Google, sign-in can't work inside any app; Google blocks that. Paste a token instead.
- If your school's login ends on its own page instead of returning to Canvas, click "Already signed in? Continue".
- If you signed in, Semester renews its own access before it expires. A pasted token can expire; when it does,
  Semester keeps your last data on screen and asks you to reconnect.
...
On a Mac you can double-click `Semester.command` instead; it runs the same setup.

- `python3 check.py`: run the checks before publishing. They also run on macOS, Windows and Linux in CI,
  and the built app is launched on each to confirm it can still draw a dashboard.
