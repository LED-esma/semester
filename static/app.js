  ...
  const seenTips = () => { try { return JSON.parse(LS.getItem('semester.tipsSeen')) || []; } catch (e) { return []; } };
  ...
  function hideTip(markSeen) {
    if (!tipEl) return;
    if (markSeen) LS.setItem('semester.tipsSeen', JSON.stringify([...new Set([...seenTips(), tipEl.dataset.id])]));
    tipEl.remove(); tipEl = tipAnchor = null;
  }
  function showTipFor(tab) {
    hideTip(false);
    if (!tipsOn() || modal.classList.contains('open')) return;
    const tip = TIPS.find(t => t.tab === tab && !seenTips().includes(t.id));
    ...
    tipEl.querySelector('button').onclick = () => hideTip(true);
  ...
  document.addEventListener('keydown', e => { if (e.key === 'Escape' && tipEl) hideTip(true); });
  document.addEventListener('tabchange', e => { hideTip(false); setTimeout(() => showTipFor(e.detail), 450); });
  ...
  if (tipsToggle) {
    ...
    tipsToggle.addEventListener('change', () => {
      ...
      if (tipsToggle.checked) { LS.removeItem('semester.tipsSeen'); showToast('Tips will show again as you use Semester.'); }
      else hideTip(false);
    });
  }
