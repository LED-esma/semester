...
<div class="wiz">
  ...
  <section class="screen" data-i="1">
    ...
    <p>Paste your Canvas address from your browser, or just type your school&rsquo;s Canvas name.</p>
    <input id="url" type="text" placeholder="yourschool.instructure.com" autocomplete="off" spellcheck="false" value="__URL__">
  ...
  <section class="screen" data-i="2">
    ...
    <div class="note" id="reauthNote" hidden>Your Canvas token expired, so Semester can&rsquo;t refresh. Make a new one and paste it below. Your settings and plans stay put.</div>
    ...
    <div class="fine">Your token is saved only on this computer.</div>
  </section>
...
<script>
  ...
  function show(n) {
    ...
    if (inp) setTimeout(() => { inp.focus(); if (inp.value) inp.select(); }, 60);  // typing replaces a pre-filled value
  }
  ...
  async function checkUrl() {
    const raw = $('url').value.trim(), st = $('urlStatus');
  ...
  async function urlNext() { clearTimeout(urlTimer); if (baseUrl || await checkUrl()) show(2); }
  // A paste is checked at once; typing waits for a short pause.
  $('url').addEventListener('input', e => { baseUrl = ''; clearTimeout(urlTimer);
    urlTimer = setTimeout(checkUrl, e.inputType === 'insertFromPaste' ? 0 : 450); });
  $('url').addEventListener('keydown', e => { if (e.key === 'Enter') urlNext(); });
  ...
  async function checkToken() {
    ...
    try {
      ...
      setStatus(st, 'Connected' + (d.name ? ' as ' + esc(d.name) : '') + '.', 'ok');
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        if self.path == "/api/reauth":
            return self._reauth()
        if self.path not in ("/api/save", "/api/check", "/api/check_url", "/api/prefs", "/api/notify", "/api/canvas/done",
